...
def main():
    ...
    parser.add_argument('--hold_time', type=int, default=5, help='Seconds to hold a detected emotion before updating to a new value')
    parser.add_argument('--font_scale', type=float, default=0.0, help='Override label font scale (0 = automatic)')
    ...
    out = er.recognise_emotion(img, return_type='BGR', downscale=downscale, hold_time=args.hold_time, font_scale=(args.font_scale if args.font_scale>0 else None))
    ...
    while True:
        ...
        if frame_idx % process_every == 0 or last_frame is None:
            ...
            try:
                last_frame = er.recognise_emotion(frame, return_type='BGR', downscale=downscale, hold_time=args.hold_time, font_scale=(args.font_scale if args.font_scale>0 else None))
            except Exception:
