...
class EmotionRecognition(object):
    ...
    def recognise_emotion(self, frame, return_type='BGR', downscale=1, hold_time=20, match_threshold=None, font_scale=None):
        ...
        if boxes is not None:
            ...
            for i in range(len(boxes)):
                ...
                # draw larger bold red label with background
                label = display_emotion
                font = cv.FONT_HERSHEY_DUPLEX
                # determine font scale: use provided font_scale if given, otherwise auto-scale by frame width
                if font_scale is None:
                    font_scale_val = max(0.7, float(f_w) / 300.0)
                else:
                    font_scale_val = float(font_scale)
                thickness = max(1, int(font_scale_val * 2))
                (tw, th), _ = cv.getTextSize(label, font, font_scale_val, thickness)
                bx2 = x1 + tw + 10
                by1 = y1 - th - 8
                by1 = max(0, by1)
                # background rectangle (red)
                frame = cv.rectangle(frame, (x1, by1), (bx2, y1), color=[0, 0, 255], thickness=-1)
                # text in white over red
                frame = cv.putText(frame, label, org=(x1 + 5, y1 - 5), fontFace=font, color=[255, 255, 255], fontScale=font_scale_val, thickness=thickness)
